...
def exec_call(args, cwd):
    ...
    try:
        ...
        print(e.stderr.decode('utf-8'))  # Added this line to print the error output
        raise
#        print(e.output)
#        print(e)
...
def generate_tf(model, logicalId, providerTypeName, providerFullName, terraformTypeName):
    """
    This function generates a Terraform (TF) configuration in JSON format.

    Parameters:
    model (dict): The model of the resource.
    logicalId (str): The logical ID of the resource.
    providerTypeName (str): The type name of the provider.
    providerFullName (str): The full name of the provider.
    terraformTypeName (str): The type name of the Terraform resource.

    Returns:
    dict: A dictionary representing the TF configuration.
    """
    ...
    if providerTypeName == "aviatrix":
        
        try:
            # Replace 'aviatrix_secret_name' with the actual name of your secret
            secret_value_response = secretsmanagerclient.get_secret_value(SecretId='aviatrix_secret')
            secret_values = json.loads(secret_value_response['SecretString'])

            aviatrix_provider_config = {
                "username": "admin",  
                "password": secret_values['password'],
                "controller_ip": secret_values['controller_ip']
            }
        except Exception as e:
            print(f"Error retrieving Aviatrix secret: {str(e)}")
            raise e
        
        tf['provider'][providerTypeName].update(aviatrix_provider_config) 
    ...
    ## Print generated TF code for debugging purposes (comment out in production to avoid exposing sensitive data in logs)    
    print(json.dumps(tf, indent=4)) 
    return tf
